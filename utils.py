...
def escape_markdown_v2(text: str) -> str:
    ...
    💡 [اصلاح]: ترتیب Escape کردن مهم است؛ ابتدا باید کاراکتر بک‌اسلش اصلی Escape شود.
    """
    text = str(text)
    
    # 1. ابتدا خود کاراکتر بک‌اسلش (\) را Escape کنید تا در مراحل بعدی دوبل نشود.
    text = text.replace('\\', '\\\\')
    
    # 2. سایر کاراکترهای رزرو شده را Escape کنید.
    reserved_chars = ['_', '*', '[', ']', '(', ')', '~', '`', '>', '#', '+', '-', '=', '|', '{', '}', '.', '!']
    for char in reserved_chars:
        # کاراکترهای رزرو شده را تنها اگر قبلاً Escape نشده‌اند، Escape کنید.
        # در این ساختار، چون اول \ را Escape کردیم، جایگزینی امن است.
