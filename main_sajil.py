...
async def run_sajil_workflow(chat_id: int, incoming_text: str):
    ...
    # 🛠️ [اصلاح اساسی]: Escape کردن ورودی کاربر با استفاده از utils
    # فرض بر این است که تابع utils.escape_markdown_v2 اضافه شده است.
    ...
    output = (
        ...
        f"شما داده‌های زیر را وارد کردید\\: `{escaped_input}`\\n"
    ...
    # 🛠️ [اصلاح]: تغییر parse_mode به MarkdownV2 و تصحیح فراخوانی کیبورد
...
# ماژول‌های sajil_part_one و sajil_part_two در این فایل اجرا نمی‌شوند
# آنها فقط برای وارد شدن تعریف شده‌اند تا منطق جریان کار را حفظ کنند.
