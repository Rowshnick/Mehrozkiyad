...
async def handle_text_message(chat_id: int, text: str):
    ...
    if step == 'AWAITING_DATE':
        ...
        if jdate:
            ...
            await save_user_state(chat_id, state) # 💡 وضعیت را بلافاصله ذخیره کنید.
...
async def handle_callback_query(chat_id: int, callback_id: str, data: str):
    ...
    # 2. هندلینگ منوی خدمات (SERVICES)
        ...
        if submenu == 'ASTRO' and param == '0': 
            ...
            # 💡 [اصلاح]: حذف بک‌اسلش‌های دستی \\(، \\)، \\.
            await utils.send_message(BOT_TOKEN, chat_id, utils.escape_markdown_v2("لطفاً تاریخ تول
